 ...
 * 3. 解析配置文件
 * 4. 创建共享内存
 * 5. 创建监听 socket，并请将其放置于 ngx_cycle_t->listening 这一动态数组中
 * 6. 初始化核心结构体 ngx_cycle_t
 * 7. 设置信号处理函数，并将 master 进程设置为 Daemon 进程
 * 8. 进入 master process 核心流程
 */
int ngx_cdecl
main(int argc, char *const *argv)
{
    ...
    // 初始化信号以及信号处理函数，也就是通过 sigaction() 系统调用完成，对于 SIGHUP、SIGSTOP、SIGTERM 等信号都完成了信号处理函数的注册
    // 相关的信号定义在 src/os/unix/ngx_process.c 的 signals 静态数组中，信号处理函数共用一个，也就是 ngx_signal_handler，
    // 同样定义在 src/os/unix/ngx_process.c 文件中:
    ...
    if (!ngx_inherited && ccf->daemon) {
        // 设置守护进程，若执行成功，那么此时 master 进程将变成守护进程，由 init 进程所接管
    ...
    // 创建 pid file，当我们使用 nginx -s reload 等方法时，nginx 会获取 pid_file 里面儿的 pid，也就是 nginx master process 的 pid
    // 然后其实就是 kill -SIGHUP process_id，nginx 命令行工具和 nginx master 进程之间的通信是通过信号完成的
